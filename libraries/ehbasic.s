...
*************************************************************************************
*																					*
*	Adapted for Y Ddraig 68000 computer by Stephen Moody							*
*																					*
*************************************************************************************

* Ver 3.60
*    Added EXIT command to return to the OS
*    Updated VEC_IN and VEC_OUT to use the OS trap functions
*    Updated VEC_LD and VEC_SV functions to load and save to file
...
*	ORG		$000400			* past the vectors in a real system
    section .text
ehbasic::
	JMP 	code_start		* C ehbasic() entry point
...
VEC_OUT
    ...
    MOVE.l	#SYS_BASIC_PUT_CHAR,d0 * get the key status
    MOVE.l  d0,syscall_command(a3) * save the syscall
    MOVE.l  d1,syscall_d0(a3)      * character in d0
    LEA     syscall_command(a3),a0 * pointer to struct
	TRAP	#14				    * do I/O function

	MOVEM.l	(sp)+,d0-d1/a0		* restore d0, d1
	RTS
...
VEC_IN
	...
	MOVE.l	#SYS_BASIC_HAS_CHAR,d0 * get the key status
    MOVE.l  d0,syscall_command(a3) * save the syscall
    LEA     syscall_command(a3),a0 * pointer to struct
	TRAP	#14				    * do I/O function
...
RETCHR
	MOVE.l	#SYS_BASIC_GET_CHAR,d0 * get the key status
    MOVE.l  d0,syscall_command(a3) * save the syscall
    LEA     syscall_command(a3),a0 * pointer to struct
    TRAP    #14
...
* LOAD routine
VEC_LD
	BSR		get_filename		* get the filename from the line
	BEQ		LAB_FCER			* if null do function call error then warm start

    * try to open the file
    MOVEM.l d1-d7/a0-a6,-(sp)
    MOVE.l  #FA_READ,d0
    MOVE.l  d0,-(sp)            * push file mode to stack
    PEA     (a1)                * push filename to stack
    PEA     g_basic_file        * push FIL struct to stack
    JSR     f_open              * call f_open code
    ADD.l   #12,sp              * clear the stack
    MOVEM.l (sp)+,d1-d7/a0-a6	* restore registers

    TST.w   d0                  * check if the file opened
    BNE.s   .LD_ERROR
...
.LD_ERROR
	BRA		LAB_FNER			* if null do function call error then warm start


* input character to register d0 from file
LOAD_in
    MOVEM.l d1-d7/a0-a6,-(sp)
	LEA     file_read(a3),a1    * push file read bytes variable to stack
    MOVE.l  a1,-(sp)
    MOVE.l  #1,-(sp)            * number of bytes to read to stack
	LEA		file_byte(a3),a1	* point to byte buffer
    MOVE.l  a1,-(sp)            * push buffer to stack
    PEA     g_basic_file        * push FIL struct to stack

	JSR	    f_read  			* read from file
    LEA     (16,sp),sp           * clear the stack
    MOVEM.l (sp)+,d1-d7/a0-a6	* restore registers

    MOVE.l  file_read(a3),d0
	TST.w	d0				    * test status
	BEQ.s	LOAD_eof		    * branch if byte read failed

	MOVE.b	file_byte(a3),d0    * get byte
...
LOAD_eof
    PEA     g_basic_file        * push FIL struct to stack
    JSR     f_close             * close the file
    ADDQ.l  #4,sp               * clear the stack

	LEA		VEC_IN(pc),a1		* get byte from input device vector
	MOVE.l	a1,V_INPTv(a3)		* set input vector
	MOVEQ	#0,d0				* clear byte

	BSR		LAB_147A			* do CLEAR, erase variables/functions and flush stacks
...
get_filename
	...
	BNE.s	.has_name			* if null go use the file requester
    RTS
...
VEC_SV
    ...
    * try to open the file
    MOVEM.l d1-d7/a0-a6,-(sp)
    MOVE.l  #FA_WRITE|FA_CREATE_ALWAYS,d0
    MOVE.l  d0,-(sp)            * push file mode to stack
    PEA     (a1)                * push filename to stack
    PEA     g_basic_file        * push FIL struct to stack
    JSR     f_open              * call f_open code
...
.SV_ERROR
	BRA		LAB_SEER			* if null do function call error then warm start
...
* in d0. these values are at the end of the .inc file
	...
	MOVEM.l d1-d7/a0-a6,-(sp)	* Save the OS registers
    MOVE.l  sp,a1               * copy the system stack pointer
...
LAB_sizok
	...
	LEA		ram_base(a3),sp		* set stack to RAM start + 4k
    MOVE.L  a1,sys_stack(a3)     * save the system stack pointer

	MOVE.w	#$4EF9,d0			* JMP opcode
	MOVEA.l	sp,a0				* point to start of vector table

	MOVE.w	d0,(a0)+			* LAB_WARM
	LEA		LAB_COLD,a1			* initial warm start vector
	MOVE.l	a1,(a0)+			* set vector

	MOVE.w	d0,(a0)+			* Usrjmp
	LEA		LAB_FCER,a1			* initial user function vector
								* "Function call" error
	MOVE.l	a1,(a0)+			* set vector

	MOVE.w	d0,(a0)+			* V_INPT JMP opcode
	LEA		VEC_IN,a1			* get byte from input device vector
	MOVE.l	a1,(a0)+			* set vector

	MOVE.w	d0,(a0)+			* V_OUTP JMP opcode
	LEA		VEC_OUT,a1			* send byte to output device vector
	MOVE.l	a1,(a0)+			* set vector

	MOVE.w	d0,(a0)+			* V_LOAD JMP opcode
	LEA		VEC_LD,a1			* load BASIC program vector
	MOVE.l	a1,(a0)+			* set vector

	MOVE.w	d0,(a0)+			* V_SAVE JMP opcode
	LEA		VEC_SV,a1			* save BASIC program vector
	MOVE.l	a1,(a0)+			* set vector

	MOVE.w	d0,(a0)+			* V_CTLC JMP opcode
	LEA		VEC_CC,a1			* save CTRL-C check vector
	MOVE.l	a1,(a0)+			* set vector
...
*##LAB_GMEM
	...
	MOVE.b	#$0,TWidth(a3)		* Unlimited width, handled by OS
...
LAB_SEER
	MOVEQ	#$32,d7			   * error code $30 "File save" error
	BRA	    LAB_XERR		   * do error #d7, then warm start

*************************************************************************************
*
* do file not found error

LAB_FNER
	MOVEQ	#$30,d7			   * error code $30 "File not found" error
...
* character $08 as delete key, BACKSPACE on standard keyboard
...
LAB_EXIT
    MOVE.l  sys_stack(a3),sp     * restore the system stack pointer
	MOVEM.l (sp)+,d1-d7/a0-a6	* restore system registers
	MOVEQ	#0,d0
	RTS							* return to OS
...
TK_EXIT     EQU TK_CLEAR+1      * $A2
TK_NEW		EQU TK_EXIT+1		* $A3
TK_WIDTH	EQU TK_NEW+1		* $A4
TK_GET		EQU TK_WIDTH+1		* $A5
TK_SWAP		EQU TK_GET+1		* $A6
TK_BITSET	EQU TK_SWAP+1		* $A7
TK_BITCLR	EQU TK_BITSET+1		* $A8
TK_TAB		EQU TK_BITCLR+1		* $A9
TK_ELSE		EQU TK_TAB+1		* $AA
TK_TO		EQU TK_ELSE+1		* $AB
TK_FN		EQU TK_TO+1			* $AC
TK_SPC		EQU TK_FN+1			* $AD
TK_THEN		EQU TK_SPC+1		* $AE
TK_NOT		EQU TK_THEN+1		* $AF
TK_STEP		EQU TK_NOT+1		* $B0
TK_UNTIL	EQU TK_STEP+1		* $B1
TK_WHILE	EQU TK_UNTIL+1		* $B2
TK_PLUS		EQU TK_WHILE+1		* $B3
TK_MINUS	EQU TK_PLUS+1		* $B4
TK_MULT		EQU TK_MINUS+1		* $B5
TK_DIV		EQU TK_MULT+1		* $B6
TK_POWER	EQU TK_DIV+1		* $B7
TK_AND		EQU TK_POWER+1		* $B8
TK_EOR		EQU TK_AND+1		* $B9
TK_OR		EQU TK_EOR+1		* $BA
TK_RSHIFT	EQU TK_OR+1			* $BB
TK_LSHIFT	EQU TK_RSHIFT+1		* $BC
TK_GT		EQU TK_LSHIFT+1		* $BD
TK_EQUAL	EQU TK_GT+1			* $BE
TK_LT		EQU TK_EQUAL+1		* $BF
TK_SGN		EQU TK_LT+1			* $C0
TK_INT		EQU TK_SGN+1		* $C1
TK_ABS		EQU TK_INT+1		* $C2
TK_USR		EQU TK_ABS+1		* $C3
TK_FRE		EQU TK_USR+1		* $C4
TK_POS		EQU TK_FRE+1		* $C5
TK_SQR		EQU TK_POS+1		* $C6
TK_RND		EQU TK_SQR+1		* $C7
TK_LOG		EQU TK_RND+1		* $C8
TK_EXP		EQU TK_LOG+1		* $C9
TK_COS		EQU TK_EXP+1		* $CA
TK_SIN		EQU TK_COS+1		* $CB
TK_TAN		EQU TK_SIN+1		* $CC
TK_ATN		EQU TK_TAN+1		* $CD
TK_PEEK		EQU TK_ATN+1		* $CE
TK_DEEK		EQU TK_PEEK+1		* $CF
TK_LEEK		EQU TK_DEEK+1		* $D0
TK_LEN		EQU TK_LEEK+1		* $D1
TK_STRS		EQU TK_LEN+1		* $D2
TK_VAL		EQU TK_STRS+1		* $D3
TK_ASC		EQU TK_VAL+1		* $D4
TK_UCASES	EQU TK_ASC+1		* $D5
TK_LCASES	EQU TK_UCASES+1		* $D6
TK_CHRS		EQU TK_LCASES+1		* $D7
TK_HEXS		EQU TK_CHRS+1		* $D8
TK_BINS		EQU TK_HEXS+1		* $D9
TK_BITTST	EQU TK_BINS+1		* $DA
TK_MAX		EQU TK_BITTST+1		* $DB
TK_MIN		EQU TK_MAX+1		* $DC
TK_RAM		EQU TK_MIN+1		* $DD
TK_PI		EQU TK_RAM+1		* $DE
TK_TWOPI	EQU TK_PI+1			* $DF
TK_VPTR		EQU TK_TWOPI+1		* $D0
TK_SADD		EQU TK_VPTR+1		* $E1
TK_LEFTS	EQU TK_SADD+1		* $E2
TK_RIGHTS	EQU TK_LEFTS+1		* $E3
TK_MIDS		EQU TK_RIGHTS+1		* $E4
TK_USINGS	EQU TK_MIDS+1		* $E5
...
LAB_BAER
    ...
    dc.w    LAB_FN-LAB_BAER         * $30 file not found
    dc.w    LAB_SE-LAB_BAER         * $32 file save error
...
LAB_FN  dc.b    'File not found',$00
LAB_SE  dc.b    'File save error',$00
...
LAB_SMSG
	...
	dc.b	'Enhanced 68k BASIC Version 3.60',$0D,$0A,$00
